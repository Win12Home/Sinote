from PySide6.QtWidgets import QPushButton, QHBoxLayout, QVBoxLayout, QLabel, QLineEdit, QSizePolicy, QWidget, QFileDialog, QMessageBox
...
class CreateProjectDialog(FramelessWindow):
    def __init__(self, parent: QWidget = None):
        ...
        self.createProjectLabel = QLabel(getLangJson("EditorUI")["editor.title.proj.createproj"])  # NOQA, against PEP 8 rules in string
        self.createProjectLabel.setSizePolicy(QSizePolicy.Policy.Fixed, QSizePolicy.Policy.Fixed)

        self.pathChooseArea = QWidget()
        self.pathChooseLayout = QHBoxLayout()
        self.pathChooseLabel = QLabel(getLangJson("EditorUI")["editor.any.path"])  # NOQA
        self.pathChooseLabel.setSizePolicy(QSizePolicy.Policy.Fixed, QSizePolicy.Policy.Fixed)
        self.pathLineEdit = QLineEdit()
        self.pathLineEdit.setPlaceholderText(getLangJson("EditorUI")["editor.placeholder.proj.enterpath"])
        self.pathChooser = QPushButton(getLangJson("EditorUI")["editor.button.proj.choose"])
        self.pathChooser.setFlat(True)
        self.pathChooser.clicked.connect(self.choosePath)
        self.pathChooser.setSizePolicy(QSizePolicy.Policy.Fixed, QSizePolicy.Policy.Fixed)
        ...
        self.projectNameLabel = QLabel(getLangJson("EditorUI")["editor.title.proj.projname"])  # NOQA
        self.projectNameLabel.setSizePolicy(QSizePolicy.Policy.Fixed, QSizePolicy.Policy.Fixed)
        self.projectNameEdit = QLineEdit()
        self.projectNameEdit.setPlaceholderText(getLangJson("EditorUI")["editor.placeholder.proj.entername"])
        ...
        self.buttonReject = QPushButton(getLangJson("MessageBox")["msgbox.button.cancel"])
    ...
    def exec(self) -> bool:  # Fake exec
        ...
        self.hide()
        if self._accepted:
            createProject(self.pathLineEdit.text(), self.projectNameEdit.text())  # Analyze Project Creating
    ...
    def accept(self) -> None:
        self._accepted = True
        if not Path(self.pathLineEdit.text()).is_dir() or self.pathLineEdit.text().strip() == "":
            QMessageBox.critical(self, getLangJson("MessageBox")["msgbox.title.error"], getLangJson("MessageBox")["msgbox.error.pathNotValid"].format(self.pathLineEdit.text().strip()), buttons=QMessageBox.StandardButton.Ok)
            return
        self.close()
    ...
    def choosePath(self) -> None:  # NOQA, f**k off PEP 8
        directory: str | None = QFileDialog.getExistingDirectory(self, getLangJson("EditorUI")["editor.window.proj.choose"])
        if directory:
